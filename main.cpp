...
#define MAX_EVENT_NUMBER 1024 // 监听最大的事件数量
#define TIMESLOT 5            // 信号时间
    ...
    while (!stop_server)
    {
        ...
        for (int i = 0; i < num; i++)
        {
            ...
            else if (events[i].events & (EPOLLRDHUP | EPOLLHUP | EPOLLERR))
            {
                ...
                // util_timer<http_conn> *timer = users[sockfd].timer;
                // cb_func(&users[sockfd]);
                // if (timer)
                // {
                //     http_conn::timer_lst.del_timer(timer);
                // }
            }
            else if (events[i].events & EPOLLIN)
            {
                ...
                if (users[sockfd].read())
                {
                    ...
                    if (timer)
                    {
                        ...
                        http_conn::timer_lst.adjust_timer(timer);
                    }
                }
                else
                {
